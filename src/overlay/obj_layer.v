...
always @(*) begin
	obj_hit = 1'b0;
	obj_type_now = {OBJ_TYPE_BITS{1'b0}};
	obj_local_x = 5'd0;
	obj_local_y = 5'd0;
	scan_obj_x = 10'd0;
	scan_obj_y = 10'd0;
	...
	for (obj_i = 0; obj_i < MAX_OBJ; obj_i = obj_i + 1) begin
		scan_obj_x = obj_x(
			...
			obj_x_bias_bus[obj_i*X_BIAS_BITS +: X_BIAS_BITS]
		);
		scan_obj_y = obj_y_bus[obj_i*OBJ_Y_BITS +: OBJ_Y_BITS];

		if (!obj_hit && obj_active_bus[obj_i] &&
...
wire [15:0] obj_rgb565 =
	obj_type_d == 2'd0 ? obj_plus1_rgb565 :
	obj_type_d == 2'd1 ? obj_plus3_rgb565 :
	obj_type_d == 2'd2 ? obj_plus5_rgb565 :
							obj_minus5_rgb565;
...
wire [SVO_BITS_PER_PIXEL-1:0] obj_or_bg_rgb =
	obj_hit_d && obj_rgb565 != TRANSPARENT_RGB565 ?
...
assign out_axis_tdata =
	...
	rgb565_to_bgr888(player_rgb565) : obj_or_bg_rgb;
